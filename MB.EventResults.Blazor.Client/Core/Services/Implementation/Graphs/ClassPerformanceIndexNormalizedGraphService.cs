...
public class ClassPerformanceIndexNormalizedGraphService(IStatsCalculator _StatsCalculator) : IClassPerformanceIndexNormalizedGraphService {
  public const string Label = "Class Performance Index (Normalized)";
  ...
  public Stat<double?> GetValues(List<Split> splits) {
    var values = splits.Select(p => p.PerformanceIndexAdjusted.HasValue ? (p.PerformanceIndexAdjusted.Value * 100).Round(1) : (double?)null).ToList();
